...
каждого pytest-сабпроцесса, так что код из чужого PR не может их прочитать. На
fork-PR всё равно включайте required approval, как для любого CI, который
гоняет чужой код.
...
момент старта были либо заброшены (Mutahunter), либо академическими
(LLMorpheus), либо закрытыми корпоративными (Meta ACH, Atlassian).
