...
## Проверено на трёх других реальных проектах
...
90 тестов, все офлайн и бесплатные: тесты пайплайна гоняют настоящие
...
- **Тексты отчёта генерирует LLM по коду, включая недоверенный.** Описание
  ...
  постятся в markdown/JSON-отчёт и в комментарий к PR как есть, без
  дополнительной санитизации. Комментарий или докстрока в PR, написанные так,
  чтобы выглядеть как инструкция модели (prompt injection), в принципе могут
  повлиять на формулировки в отчёте. Учитывайте это, когда Action гоняется на
