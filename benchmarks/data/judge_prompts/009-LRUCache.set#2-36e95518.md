...
The mutation that survived the test suite (state_leak): Overwriting an existing key's value does not refresh its recency, so it can be evicted as if it hadn't been touched recently.
