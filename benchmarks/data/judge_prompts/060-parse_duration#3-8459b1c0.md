...
The mutation that survived the test suite (boundary_condition): Passing an empty string after stripping (e.g. all-whitespace input) no longer raises immediately and instead falls through, eventually raising a different, less specific error about no duration found
