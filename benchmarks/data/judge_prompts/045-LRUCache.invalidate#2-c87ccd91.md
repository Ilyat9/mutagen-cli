<!-- id: LRUCache.invalidate#2-c87ccd91 -->
...
The mutation that survived the test suite (state_leak): invalidate() removes the key from storage but leaves it in the LRU order tracking, so stale key references can linger and affect eviction ordering
