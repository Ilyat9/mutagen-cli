<!-- id: LRUCache.__len__#3-07b4ae7c -->
...
The mutation that survived the test suite (off_by_one): len(cache) overcounts by one, so callers relying on the reported size see one more entry than actually stored.
