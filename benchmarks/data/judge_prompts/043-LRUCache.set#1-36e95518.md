...
The mutation that survived the test suite (missing_invalidation): Setting a value for a key that already exists no longer refreshes its recency, so it can be evicted as if it were the oldest entry even though it was just updated.
