<!-- id: LRUCache.invalidate_prefix#2-9448ca3c -->
...
The mutation that survived the test suite (wrong_operator): invalidate_prefix matches keys by suffix instead of prefix, so it silently fails to invalidate the keys callers actually meant to clear and may clear unrelated ones.

```
-         doomed = [key for key in self._data if key.startswith(prefix)]
+         doomed = [key for key in self._data if key.endswith(prefix)]
```
