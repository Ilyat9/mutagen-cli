<!-- id: LRUCache.__init__#1-9dcd3d4c -->
...
The mutation that survived the test suite (boundary_condition): A cache created with capacity=0 is accepted instead of rejected, so callers get a cache that silently can never store anything.
