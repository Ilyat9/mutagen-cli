<!-- id: LRUCache.invalidate#1-f24f58ef -->
...
The mutation that survived the test suite (wrong_default): Invalidating a key that was never cached returns None instead of False, so callers checking the boolean result of invalidate() get an unexpected falsy-but-wrong value that can break stricter checks like `is False`.
