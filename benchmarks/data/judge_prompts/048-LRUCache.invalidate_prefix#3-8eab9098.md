<!-- id: LRUCache.invalidate_prefix#3-8eab9098 -->
...
The mutation that survived the test suite (wrong_operator): invalidate_prefix only removes keys that exactly match the prefix elsewhere in the string rather than keys starting with it, so prefix-based invalidation of nested namespaces silently fails
