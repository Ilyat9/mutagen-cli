<!-- id: LRUCache.__len__#1-16036d8b -->
...
The mutation that survived the test suite (off_by_one): len(cache) undercounts by one, so callers relying on the reported size see one fewer entry than actually stored.
