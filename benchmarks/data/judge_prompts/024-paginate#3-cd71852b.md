...
The mutation that survived the test suite (other): The page's reported total item count is the size of the current slice instead of the whole collection, so has_next/has_prev and downstream page counts silently become wrong once results span more than one page.
