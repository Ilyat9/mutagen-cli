...
The mutation that survived the test suite (wrong_operator): Whitespace-only input like a string of spaces is treated as empty before stripping is checked, but leading/trailing whitespace inside mixed durations like '1h 30m' now silently drops the space instead of raising, changing which strings are accepted vs rejected inconsistently
