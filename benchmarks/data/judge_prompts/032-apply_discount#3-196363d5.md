...
The mutation that survived the test suite (other): The discounted price is rounded to one decimal place instead of two cents of precision, silently losing sub-cent accuracy that downstream billing code expects.

```
-     return round(price - discount, 2)
+     return round(price - discount, 1)
```
