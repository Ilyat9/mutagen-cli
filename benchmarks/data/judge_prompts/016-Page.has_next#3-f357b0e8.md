...
The mutation that survived the test suite (argument_order): has_next computes the page count with the arguments swapped, producing a wildly wrong result whenever total and per_page differ, so pagination stops or continues at the wrong point.

```
-         return self.page < page_count(self.total, self.per_page)
+         return self.page < page_count(self.per_page, self.total)
```
