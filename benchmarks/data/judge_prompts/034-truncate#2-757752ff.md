...
The mutation that survived the test suite (boundary_condition): When the limit is smaller than the suffix itself, truncate returns the full suffix instead of trimming it to fit, so the result can exceed the requested limit.
