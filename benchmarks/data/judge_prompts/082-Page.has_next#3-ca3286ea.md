...
The mutation that survived the test suite (inverted_condition): has_next reports whether there is a previous page instead of a next one.

```
-         return self.page < page_count(self.total, self.per_page)
+         return self.page > 1 and self.page <= page_count(self.total, self.per_page)
```
